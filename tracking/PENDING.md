...
- [x] **Optimización de control de stock, filtros avanzados, ordenación y alertas de pedidos en admin** (2026-09-19, Lead Developer (Antigravity)): buscador reactivo por texto y EAN-13, selector de categorías dinámicas, selector de ordenación multicriterio (stock asc/desc, precio, nombre, categoría), píldoras de estado con contadores vivos (Agotados, Crítico/Bajo, Comprometidos, Saludables), tarjetas KPI superiores (`InventoryKpiSummary`), desglose de stock físico vs stock comprometido en pedidos (`calculateCommittedStock`) vs disponible real con modal de detalle, botones de reposición rápida (+1, +5, +10), paginación local de tabla (25/50/100/500), badges de urgencia temporal en pedidos (`OrderUrgencyBadge` y `getOrderUrgency`) con priorización de pedidos de recogida en tienda y envíos pendientes. Verificado con `npm run build` y `bash verify.sh`.
